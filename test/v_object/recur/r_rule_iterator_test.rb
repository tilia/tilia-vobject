...
module Tilia
  module VObject
    class RRuleIteratorTest < Minitest::Test
      def parse(rule, start, expected, fast_forward = nil, tz = 'UTC')
        tz = ActiveSupport::TimeZone.new(tz)
        dt = tz.parse(start)
        parser = Tilia::VObject::Recur::RRuleIterator.new(rule, dt)

        parser.fast_forward(tz.parse(fast_forward)) if fast_forward

        result = []
        while parser.valid
          item = parser.current
          result << item.strftime('%Y-%m-%d %H:%M:%S')

          break if parser.infinite? && result.size >= expected.size
          parser.next
        end
      ...
      def test_fifth_tuesday_problem
        parse(
      ...
      def test_until_begin_has_timezone
        parse(
      ...
      def test_until_before_dt_start
        parse(
      ...
      def test_iterator_functions
        parser = Tilia::VObject::Recur::RRuleIterator.new('FREQ=DAILY', Time.zone.parse('2014-08-02 00:00:13'))
        parser.next
        assert_equal(Time.zone.parse('2014-08-03 00:00:13'), parser.current)

        assert_equal(1, parser.key)

        parser.rewind

        assert_equal(Time.zone.parse('2014-08-02 00:00:13'), parser.current)

        assert_equal(0, parser.key)
      end
    end
  end
end
